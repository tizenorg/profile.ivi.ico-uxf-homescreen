...
void CicoAilItems::init()
{
    ...
    m_hideWait = false;         // wait at surface hide
...
const string s_hidenowait("hidenowait");
const string s_hidewait("hidewait");
...
// TODO mk_k not get switchName
        ...
        // hidewait
        if (0 == strncasecmp(it->c_str(), s_hidewait.c_str(),
                             s_hidewait.size())) {
            if (true == categoryParseHideWait(*it)) {
                ICO_DBG("Parse OK %s", it->c_str());
                continue;
            }
        }
        ...
        // hidenowait
        if (0 == strncasecmp(it->c_str(), s_hidenowait.c_str(),
                             s_hidenowait.size())) {
            if (true == categoryParseHideNoWait(*it)) {
                ICO_DBG("Parse OK %s", it->c_str());
                continue;
            }
        }
 ...
 * @retval false parse fail
 */
bool CicoAilItems::categoryParseHideWait(const string&)
{
    ICO_TRA("start");
    m_hideWait = true;
    ICO_TRA("end");
...
bool CicoAilItems::categoryParseHideNoWait(const string&)
{
    ICO_TRA("start");
    m_hideWait = false;
    ICO_TRA("end");
