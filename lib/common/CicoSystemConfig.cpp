...
#define ICO_SYC_CONFIG_DIR  "res/config"
...
CicoSystemConfig::createUserConf(const ptree & root)
{
    ...
    if (true == opts.is_initialized()) {
        ...
        if (! v.empty()) {
            char wkbuf[256];
            if (CicoGKeyFileConfig::ConfigTizenPlatform(v.c_str(), wkbuf, sizeof(wkbuf))
                == 0)   {
                m_userConf->m_parent_dir = wkbuf;
            }
        }
    }
    if (m_userConf->m_parent_dir.empty())   {
        m_userConf->m_parent_dir = tzplatform_mkpath(TZ_USER_HOME, ICO_SYC_CONFIG_TOPDIR);
    }
    m_userConf->dumpConf();
}
